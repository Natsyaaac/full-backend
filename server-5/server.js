...
app.delete('/api/bugs/:id', async (req, res) => {
  try {
    ...
    if (isNaN(bugId)) {
      return res.status(400).json(errorResponse(null, 'ID tidak valid'));
    }
    const newPostBugs = bugs.filter(bug => bug.id !== bugId);

    if(newPostBugs.length === bugs.length) {
      throw new Error('Postingan tidak ditemukan')
    }

    bugs = newPostBugs
    res.json(successResponse(NewPostBugs));
  } catch (error) {
    res.status(500).json(errorResponse(null, error.message))
  }
});
